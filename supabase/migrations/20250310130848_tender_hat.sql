/*
  # Naprawa systemu rezerwacji

  1. Zmiany
    - Dodaje brakujący szablon emaila
    - Dodaje logowanie błędów rezerwacji
    - Poprawia obsługę statusów rezerwacji
  
  2. Bezpieczeństwo
    - Zachowuje istniejące uprawnienia
*/

-- Dodaj tabelę do logowania błędów rezerwacji
CREATE TABLE IF NOT EXISTS reservation_errors (
...
-- Dodaj uprawnienia dla tabeli błędów
ALTER TABLE reservation_errors ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can view errors"
    ON reservation_errors
    FOR SELECT
    TO authenticated
    USING (
        EXISTS (
...
-- Dodaj lub zaktualizuj szablon emaila
