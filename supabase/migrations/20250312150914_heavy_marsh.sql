/*
  ...
  1. Changes
    ...
    - Add force_status parameter
    - Add validation for reversed status changes
*/
...
CREATE OR REPLACE FUNCTION update_reservation_status(
  ...
  p_force_status boolean DEFAULT false
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Jeśli to przejście wsteczne i nie wymuszono zmiany, wymagaj komentarza
  IF v_is_reversed AND NOT p_force_status AND p_comment IS NULL THEN
