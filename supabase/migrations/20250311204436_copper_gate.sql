...
BEGIN
  ...
  v_start_timestamp := date_trunc('day', v_start_local) + (v_start_hour || ' hours')::interval;
  v_end_timestamp := date_trunc('day', v_end_local) + (v_end_hour || ' hours')::interval;

  -- Calculate base number of days
  v_days := CASE
    -- If end time is 8:00, don't count the last day
    WHEN v_end_hour = 8 THEN
      CEIL(EXTRACT(EPOCH FROM (v_end_timestamp - interval '1 day' - v_start_timestamp))/86400.0)::integer
    ELSE
      CEIL(EXTRACT(EPOCH FROM (v_end_timestamp - v_start_timestamp))/86400.0)::integer
    END;
