/*
  # Update SPF record in SMTP settings

  1. Changes
    - Update SPF record in SMTP settings comment
    - Keep other DNS records unchanged
*/

-- Update SMTP settings comment with new SPF record
COMMENT ON TABLE smtp_settings IS E'SMTP Configuration\n\nDKIM: v=DKIM1; k=rsa; p=\n\nSPF: v=spf1 redirect=_spf-h22.seohost.pl\n\nDMARC: v=DMARC1; p=none; sp=none; rua=mailto:spam-reports@microhost.pl\n\nMX: 10 mail.solrent.pl';